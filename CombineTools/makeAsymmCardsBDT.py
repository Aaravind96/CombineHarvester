...
for channel in channels:
    for massList in allMasses[signalType]:
        for masspoint in massList:
            if channel == "emu":
                command=f"hToA1A2_modified {channel} {signalType} {year} {masspoint[0]} {masspoint[1]} 5 SR1_1b SR2_1b SR3_1b SR1_2b SR2_2b"
            else:
                command=f"hToA1A2_modified {channel} {signalType} {year} {masspoint[0]} {masspoint[1]} 6 SR1_1b SR2_1b SR3_1b SR4_1b SR1_2b SR2_2b"
            os.system(command)

        os.system("mv hToA1A2*.txt asymmCards/signal2b2t_new/bdt")
        os.system("mv hAsymm*.root asymmCards/signal2b2t_new/bdt")

        for masspoint in massList:
            ch1_filename   = f"asymmCards/signal2b2t_new/bdt/hToA1A2_{channel}_1_{year}_{signalType}_{masspoint[0]}_{masspoint[1]}.txt"
            ch2_filename   = f"asymmCards/signal2b2t_new/bdt/hToA1A2_{channel}_2_{year}_{signalType}_{masspoint[0]}_{masspoint[1]}.txt"
            ch3_filename   = f"asymmCards/signal2b2t_new/bdt/hToA1A2_{channel}_3_{year}_{signalType}_{masspoint[0]}_{masspoint[1]}.txt"
            ch4_filename   = f"asymmCards/signal2b2t_new/bdt/hToA1A2_{channel}_4_{year}_{signalType}_{masspoint[0]}_{masspoint[1]}.txt"
            ch5_filename   = f"asymmCards/signal2b2t_new/bdt/hToA1A2_{channel}_5_{year}_{signalType}_{masspoint[0]}_{masspoint[1]}.txt"
            if channel == "mutau" or channel == "etau":
                ch6_filename   = f"asymmCards/signal2b2t_new/bdt/hToA1A2_{channel}_6_{year}_{signalType}_{masspoint[0]}_{masspoint[1]}.txt"
            allch_filename = f"asymmCards/signal2b2t_new/bdt/hToA1A2_{signalType}_{channel}_{year}_{masspoint[0]}_{masspoint[1]}.txt"
            ...
            os.system(f'echo "* autoMCStats 10 0" >> {ch4_filename}')
            os.system(f'echo "* autoMCStats 10 0" >> {ch5_filename}')
            if channel == "mutau" or channel == "etau":
                os.system(f'echo "* autoMCStats 10 0" >> {ch6_filename}')

            if channel == "mutau" or channel == "etau":
                os.system(f'combineCards.py {ch1_filename} {ch2_filename} {ch3_filename} {ch4_filename} {ch5_filename} {ch6_filename}> {allch_filename}')
            else: 
                os.system(f'combineCards.py {ch1_filename} {ch2_filename} {ch3_filename} {ch4_filename} {ch5_filename}> {allch_filename}')
