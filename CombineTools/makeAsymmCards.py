...
for channel in channels:
    for massList in allMasses[signalType]:
        ...
        os.system("mv hToA1A2*.txt asymmCards/cutbased")
        os.system("mv hAsymm*.root asymmCards/cutbased")
        ...
        for masspoint in massList:
            ...
            allch_workspace_rootfile = f"asymmCards/cutbased/hToA1A2_{signalType}_{channel}_{year}_{masspoint[0]}_{masspoint[1]}.root"
            logfile = f"asymmCards/cutbased/limits_{signalType}_{channel}_{year}_{masspoint[0]}_{masspoint[1]}"
            ...
            os.system(f"mv {oldName} asymmCards/cutbased/{newName}")
        ...
        filesToHadd = ""
        for masspoint in massList:
            filesToHadd += f"asymmCards/cutbased/higgsCombineTest.AsymptoticLimits.{signalType}_{channel}_{year}.m2_{masspoint[0]}.m1_{masspoint[1]}.root"
            filesToHadd += " "
        print(filesToHadd)
        # add the signalType to the file name, to distinguish the cascade and non-cascade ones
        os.system(f'hadd -f -j -k asymmCards/cutbased/higgsCombine_a1a2_{signalType}_{channel}_{year}_m1_{thisM1Point}.root {filesToHadd}')
