...
for massList in allMasses[signalType]:
    for masspoint in massList:
        ...
        os.system(f'cd asymmCards/signal2b2t_new/bdt; combineCards.py hToA1A2_{signalType}_mutau_{year}_{masspoint[0]}_{masspoint[1]}.txt  hToA1A2_{signalType}_etau_{year}_{masspoint[0]}_{masspoint[1]}.txt hToA1A2_{signalType}_emu_{year}_{masspoint[0]}_{masspoint[1]}.txt > hToA1A2_{signalType}_allchannels_{year}_{masspoint[0]}_{masspoint[1]}.txt; cd ../../../')
