...
for massList in allMasses[signalType]:
    for masspoint in massList:
        ...
        os.system(f'cd asymmCards/; combineCards.py hToA1A2_{signalType}_allchannels_2018_{masspoint[0]}_{masspoint[1]}.txt hToA1A2_{signalType}_allchannels_2017_{masspoint[0]}_{masspoint[1]}.txt > hToA1A2_{signalType}_allchannels_allyears_{masspoint[0]}_{masspoint[1]}.txt; cd ../')
    ...
    for masspoint in massList:
        ...
        if (doBlindedLimits):  # -t -1 option
            combineCommand = f'combine -M AsymptoticLimits asymmCards/hToA1A2_{signalType}_allchannels_allyears_{masspoint[0]}_{masspoint[1]}.root --rMin=-10 -t -1 -m {masspoint[0]} | tee asymmCards/limits_allchannels_{masspoint[0]}_{masspoint[1]}'
        else: 
            combineCommand = f'combine -M AsymptoticLimits asymmCards/hToA1A2_{signalType}_allchannels_allyears_{masspoint[0]}_{masspoint[1]}.root -m {masspoint[0]} | tee asymmCards/limits_allchannels_{masspoint[0]}_{masspoint[1]}'
    ...
    print(filesToHadd)
    os.system(f'hadd -f -j -k asymmCards/higgsCombine_a1a2_{signalType}_allchannels_allyears_m1_{thisM1Point}.root {filesToHadd}')
