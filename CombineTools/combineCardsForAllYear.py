# Usage:
#   python3 combineCardsForAllYear.py
#
...
for massList in allMasses[signalType]:
    for masspoint in massList:
        ...
        os.system(f'cd asymmCards/; combineCards.py hToA1A2_allchannels_allcats_2018_{masspoint[0]}_{masspoint[1]}.txt hToA1A2_allchannels_allcats_2017_{masspoint[0]}_{masspoint[1]}.txt > hToA1A2_allchannels_allcats_allyears_{masspoint[0]}_{masspoint[1]}.txt; cd ../')

        # Convert text to workspace
        print(f"Converting .txt to workspace....")
        os.system(f'text2workspace.py asymmCards/hToA1A2_allchannels_allcats_allyears_{masspoint[0]}_{masspoint[1]}.txt -m {masspoint[0]}')

    for masspoint in massList:
        print(f"Creating higgsCombineTest.AsymptoticLimits.*.root....")
        if (doBlindedLimits):  # -t -1 option
            combineCommand = f'combine -M AsymptoticLimits asymmCards/hToA1A2_allchannels_allcats_allyears_{masspoint[0]}_{masspoint[1]}.root --rMin=-10 -t -1 -m {masspoint[0]} | tee asymmCards/limits_allchannels_{masspoint[0]}_{masspoint[1]}'
        else: 
            combineCommand = f'combine -M AsymptoticLimits asymmCards/hToA1A2_allchannels_allcats_allyears_{masspoint[0]}_{masspoint[1]}.root -m {masspoint[0]} | tee asymmCards/limits_allchannels_{masspoint[0]}_{masspoint[1]}'
    ...
    print(filesToHadd)
    os.system(f'hadd -f -j -k asymmCards/higgsCombine_a1a2_{signalType}_allyears_allchannels_m1_{thisM1Point}.root {filesToHadd}')

    user=os.environ['USER']
    firstletteruser=user[0]

    suggested_command = f"cp asymmCards/higgsCombine_a1a2_{signalType}_allyears_allchannels*.root /afs/cern.ch/work/{firstletteruser}/{user}/public/hToA1A2/CMSSW_13_2_6_patch2/src/lunaFramework/limits/"
